...
The flow:
    ...
    5. GET  /api/publish/reports/{id}/export     take it away as Markdown or HTML
       POST /api/publish/reports/{id}/google-docs   or as a Google Doc
...
send endpoint here would break that guarantee, not extend it. The Google Docs
export keeps it: the Doc is created private to the connected account's Drive
and MIOS never shares it — a person still decides who reads it.
...
def remove_report(
    ...
    if not delete_report(report_id):
        raise HTTPException(status_code=404, detail="No such report.")
    google_docs.forget_export(report_id)
...
def _to_docs_html(report: dict[str, Any]) -> str:
    """The report as HTML for Drive to convert into a Google Doc."""
    from html import escape
...
def google_doc_status(
    ...
    """Whether Google Docs is connected, and this report's Doc if it has one."""
    _found(get_report(report_id))
    st = google_docs.status()
    return {"connected": st["connected"],
            "account": (st["account"] or {}).get("email") if st["connected"] else None,
            "export": google_docs.export_record(report_id)}


@router.post("/reports/{report_id}/google-docs")
def export_to_google_docs(
    report_id: str,
    user: dict[str, Any] = Depends(require_user),
) -> dict[str, Any]:
    """Create the report's Google Doc, or replace the content of the one it has."""
    report = _found(get_report(report_id))
    try:
        result = google_docs.export(report, _to_docs_html(report), by=user["email"])
    except google_docs.GoogleDocsNotConfigured as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except google_docs.GoogleDocsError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return {"connected": True, "export": result}
