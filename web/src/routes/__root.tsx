...
function UserMenu() {
  ...
  return (
    <div className="who-menu" ref={ref}>
      ...
      {open && (
        <div
          className="who-dropdown"
          role="menu"
        >
          <div className="who-identity">
            <div className="who-fullname">
              {user.name}
            </div>
...
function Shell({ children }: { children: ReactNode }) {
  ...
  /*
   * NEW:
   * Store the real number of Watchlist companies.
   */
  ...
  /*
   * NEW:
   * Ask the backend for the real Watchlist count.
   */
  useEffect(() => {
    const apiBase =
    ...
    fetch(`${apiBase}/api/watchlist`, {
      ...
      .then((res) => {
        if (!res.ok) {
          throw new Error(
            `Watchlist request failed: ${res.status}`,
          )
        }
  ...
  // Expanded is the default, and the server always renders that.
  ...
  useEffect(() => {
    try {
      ...
      // Private mode or blocked storage — default is fine.
    }
  }, [])
    ...
    <>
      {session?.authDisabled && (
        <div className="auth-off-bar">
          AUTH_DISABLED — sign-in is bypassed.
          Development only.
        </div>
      )}
      ...
      <div className={shellClass}>
        <div className="topbar">
          ...
          <div
            ...
            title="Easy Skill Australia · Australia and Papua New Guinea"
          >
            EASY SKILL · AU · PNG
          </div>
            ...
            >
              ...
              {g.items.map((it) => (
                ...
                >
                  ...
                  {/*
                    Watchlist gets its real count
                    from the API.

                    All other badges continue using
...
// One QueryClient for the browser; router context
// provides its own per request on the server.
